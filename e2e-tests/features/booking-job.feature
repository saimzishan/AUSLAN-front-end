Feature: Booking Admin Management
  ...
  @runThis
  Scenario: As INTERPRETER, I cannot accept booking to allocated, but admin can UnAssign and ReAssign that, if a booking with two interpreters is created and INTERPRETER_ALL Invited and Administrator exists then
    ...
    Then I see 1 interpreter has accepted the booking
    And I can see the booking state 'In Progress'
    And I click on BUTTON name 'unassingBtn_02'
    Then I can see the button 'Save' is enabled
    And I click on BUTTON 'Save'
    Then I wait for 1000 milli-seconds
    Then I get valid message: 'The interpreter have been unassigned'
  ...
  @runThis
  Scenario: As INTERPRETER, I have not accept or decline, but admin can Assign directly, if a booking with two interpreters is created and INTERPRETER and INTERPRETER1 exists and INTERPRETER_ALL Invited then
    And I sign in with valid Administrator credentials
    When I am on the bookings page
    Then I see one row with state 'In progress'
    Then I click on an individual booking of type 'In progress'
